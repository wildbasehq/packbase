...
import {useResourceStore} from '@/lib/states.ts'

export default function ResourceSettingsMembers() {
    const {currentResource: {id}} = useResourceStore()
    const [members, setMembers] = useState([])

    useEffect(() => {
        vg.pack({id}).members.get().then(({data}) => {
            setMembers(data)
        })
    }, [])
                ...
                <TableBody>
                    {members.map((member) => (
                        <TableRow key={member.id}>
                            <TableCell><UserInfo user={member}/></TableCell>
                            <TableCell>{Intl.DateTimeFormat(navigator.language, {
                                weekday: 'long',
                                month: 'short',
                                day: 'numeric',
                                hour: 'numeric',
                                minute: 'numeric',
                                second: 'numeric',
                            }).format(new Date(member.joined_at))}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
