...
import { ContentBlock, TableData } from '@/types/terms';
...
const ContentRenderer: React.FC<ContentRendererProps> = ({
  ...
  const renderContent = () => {
    switch (block.type) {
      case 'heading':
        const HeadingSize = block.level === 1 ? '3xl' :
                           block.level === 2 ? '2xl' : 'xl';
        return (
          <Heading
            size={HeadingSize}
            className={`mb-4 font-semibold ${block.className || ''}`}
          >
            {block.content}
          </Heading>
        );
      ...
      case 'text':
        return (
          <Text className={`leading-relaxed ${block.className || ''}`}>
            {block.content}
          </Text>
        );
      ...
      case 'table':
        if (block.tableData?.rows === 'dataCollection') {
          ...
          const sortedData = [...dataCollection].sort((a, b) =>
            retentionOrder.indexOf(a.retention) - retentionOrder.indexOf(b.retention)
          );
