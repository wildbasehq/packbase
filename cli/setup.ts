...
async function installDependencies(): Promise<void> {
  await tasuku("Installing workspace dependencies", async ({ setTitle }) => {
...
async function startPostgresContainer(): Promise<void> {
  await tasuku("Starting PostgreSQL container", async ({ setTitle, setStatus }) => {
...
async function startMinIOContainer(config: SetupConfig): Promise<void> {
  await tasuku("Starting MinIO container", async ({ setTitle, setStatus }) => {
...
function generateServerEnv(config: SetupConfig): string {
  ...
  if (config.rheoEnabled) {
    env.push(
      "# Rheo Moderation",
      "RHEO_ENABLED=true",
      `RHEO_ENDPOINT=${config.rheoEndpoint}`,
      `RHEO_API_KEY=${config.rheoApiKey}`,
      "",
    );
  }
...
async function main() {
  ...
  if (config.storageType === "minio") {
    await startMinIOContainer(config);
  }
