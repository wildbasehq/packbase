...
export default function PagedContainer({ pages, hasMore, children, loader, endMessage, onNeedsContent }: PagedContainerType) {
    ...
    useEffect(() => {
        if (!!pages && page > pages) return setSearchParams({ page: '1' })
        setLoading(true)
    ...
    return (
        <div>
            ...
            {loading && loader && <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full">{loader}</div>}
            {!hasMore && endMessage && <div className="py-8 text-center text-muted-foreground dark:text-neutral-400">{endMessage}</div>}
            <Pagination>
