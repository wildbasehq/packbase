...
// Combined hook for message editing (only re-renders when editing state changes)
export const useMessageEditing = () =>
  useChatStore((state) => ({
    editingMessageId: state.editingMessageId,
    editContent: state.editContent,
    isEditing: (messageId: string) => state.editingMessageId === messageId,
    startEdit: (messageId: string, content: string) => 
      state.setEditingMessage(messageId, content),
    updateContent: state.setEditContent,
    cancelEdit: state.cancelEdit,
  }))