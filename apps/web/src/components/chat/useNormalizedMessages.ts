...
// Hook for getting a specific message by ID (memoized)
export const useMessageById = (normalizedMessages: NormalizedMessages, messageId: string) => {
  return useMemo(() => {
    return normalizedMessages.byId[messageId] || null
  }, [normalizedMessages.byId, messageId])
}

// Hook for getting messages by author ID (memoized)
export const useMessagesByAuthor = (normalizedMessages: NormalizedMessages, authorId: string) => {
  return useMemo(() => {
    return normalizedMessages.allIds
      .map(id => normalizedMessages.byId[id])
      .filter(msg => msg.author_id === authorId)
  }, [normalizedMessages.byId, normalizedMessages.allIds, authorId])
}