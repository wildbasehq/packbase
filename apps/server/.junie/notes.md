...
## Overview
Yapock appears to be a social platform server with features for user profiles, content sharing ("howls"), themed interfaces, and group-based organization ("packs"). The server is built with modern TypeScript technologies and follows a structured API design.
...
## Recent Changes
There appears to be a migration from Supabase Auth to Clerk for authentication, as evidenced by the migration file `src/migrate/1_supabase_auth_to_clerk.ts`.
