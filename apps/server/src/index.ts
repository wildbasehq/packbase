...
setTimeout(async () => {
    ...
    const logStartup = () => {
        ...
        if (startupMs > 150) {
            log.info(`Server \x1b[31mSLOW\x1b[0m: Startup took ${startupMs}ms (true ${performance.now()}) (${Math.round(startupMs / 1000)}s${(Math.round(startupMs / 1000) > 60 ? `, ${Math.round(startupMs / 1000 / 60)}m` : '')}), Listening on ${process.env.PORT || 8000}\nThis took too long! It will add up the more server instances you have.`)
        } else {
