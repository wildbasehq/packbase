...
export default (app: YapockType) =>
    app.get(
        ...
        async ({params: {id, scope}, user}) => {
            ...
            if (!pack) throw HTTPError.notFound({
                ...
                if (page.ticker) {
                    ...
                    try {
                        ...
                        if (posts && posts.length > 0) {
                            const post = posts[0]
                            page.ticker = post.body.replace(/<[^>]+>/g, '').replace(/&nbsp;/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 100)
                        } else {
                            page.ticker = null
                        }
                    } catch (error) {
...
export async function getPackMembership(packId: string, userId: string) {
    ...
    try {
        ...
        if (packData?.owner_id === userId) {
            membership.permissions = pack.PERMISSIONS.Owner
        }
    } catch (error) {
        // If there's an error, we'll just continue without setting permissions
    }
